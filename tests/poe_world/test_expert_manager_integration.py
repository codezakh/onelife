...
def test_expert_manager_protocol_implementation():
    """
    Test that ExpertManager properly implements ExpertManagerProtocol.

    This test validates that ExpertManager can be used wherever
    ExpertManagerProtocol is expected.
    """
    observable_extractor = ObservableExtractor()
    weight_fitter = MaxLikelihoodWeightFitter(observable_extractor)

    # Create ExpertManager instance
    manager = ExpertManager(observable_extractor, weight_fitter)

    # Verify it can be assigned to protocol type
    protocol_manager: ExpertManagerProtocol[GameState, Action] = manager

    # Verify all protocol methods are callable
    assert callable(protocol_manager.add_experts)
    assert callable(protocol_manager.fit_weights)
    assert callable(protocol_manager.prune_experts)
    assert callable(protocol_manager.evaluate_log_probability)
    assert callable(protocol_manager.get_experts)
    assert callable(protocol_manager.save)
    assert callable(protocol_manager.load)

    # Test that methods work correctly
    experts = [WeightedExpert(expert_function=CORRECT_EXPERTS[0], weight=1.0)]
    protocol_manager.add_experts(experts)

    assert len(protocol_manager.get_experts()) == 1
    assert protocol_manager.get_experts()[0].weight == 1.0


def test_expert_manager_error_propagation():
    """
    Test that ExpertManager properly propagates errors from underlying components.

    This test validates that errors from the weight fitter or world model
    are not caught and handled by ExpertManager, but instead propagate up
    to the caller as specified in the requirements.
    """
    observable_extractor = ObservableExtractor()
    weight_fitter = MaxLikelihoodWeightFitter(observable_extractor)
    manager = ExpertManager(observable_extractor, weight_fitter)
    ...
    # Test with invalid transitions (should propagate errors)
    invalid_transitions = [
        SymbolicTransition(
            prev_metadata="invalid_state",  # Wrong type
            action=Action.MOVE_LEFT,
            next_metadata="invalid_state",
        )
    ]

    # This should raise an error from the underlying components
    with pytest.raises(Exception):
        manager.fit_weights(invalid_transitions)

    # Test with invalid state for evaluation (should propagate errors)
    with pytest.raises(Exception):
        manager.evaluate_log_probability(
            "invalid_state", Action.MOVE_LEFT, "invalid_state"  # Wrong type
        )
