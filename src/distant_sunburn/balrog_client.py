...
class LlmClientConfig(BaseModel):
    ...
    generate_kwargs: GenerateKwargs = GenerateKwargs()
...
class LLMClientWrapper:
    ...
    def __init__(self, client_config: LlmClientConfig):
        ...
        self.client_kwargs = client_config.generate_kwargs.model_dump()
    ...
    def get_completion(self, converted_messages, max_retries=5, delay=5):
        ...
        while retries < max_retries:
            try:
                ...
                retries += 1
                logger.error(
                    f"Retryable error during generate_content: {e}. Retry {retries}/{max_retries}"
                )
        ...
        raise Exception(
            f"Failed to get a valid completion after {max_retries} retries."
        )
...
class ClaudeWrapper(LLMClientWrapper):
    ...
    def convert_messages(self, messages: list[Message]) -> list[dict]:
        ...
        for msg in messages:
            ...
            if msg.attachment is not None:
                converted_messages[-1]["content"].append(
                    process_image_claude(msg.attachment)
                )
...
def make_llm_client_factory(
    ...
    def client_factory() -> LLMClientWrapper:
